...
def extract_data(invoicefile, templates=None, input_module=pdftotext):
    ...
    # print(templates[0])
    logger.debug("Testing filneame: {} ".format(invoicefile))
    ...
    for t in templates:
        ...
        if t.matches_input(optimized_str):
            exe = t.extract(optimized_str)
            if exe != None:        
                exe['template_name'] = t['template_name']
            return exe
...
def main(args=None):
    ...
    for f in args.input_files:
        ...
        if res:
            for r in res: #Umwandlung Tupel/Liste in string
                if type(res[r]) is tuple: 
                    res[r] = ' '.join(res[r])
                    logger.info(res[r])
                if type(res[r]) is list:
                    res[r] = ' '.join(chain(*res[r]))
                    logger.info(res[r])
            logger.info(res)
            output.append(res)
            if args.copy:
